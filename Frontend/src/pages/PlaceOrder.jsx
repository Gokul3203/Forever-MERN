  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      switch (method) {
        ...
        case 'cod':
          const response = await axios.post(backendUrl + 'api/oredr/place', orderData, { headers: { token } })
  ...
  return (
    <form className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
        ...
        <input onChange={onChangeHandler} name='email' value={formData.email} className='border border-gray-300 py-1.5 px-3.5 w-full' type="email" placeholder='Email address' required />
        <input onChange={onChangeHandler} name='street' value={formData.street} className='border border-gray-300 py-1.5 px-3.5 w-full' type="text" placeholder='Street' required />
        <div className='flex gap-3'>
        ...
        <div className='flex gap-3'>
          <input onChange={onChangeHandler} name='zipCode' value={formData.zipCode} className='border border-gray-300 py-1.5 px-3.5 w-full' type="number" placeholder='Zipcode' required />
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            ...
            <div onClick={() => setMethod('razaro')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razaro' ? 'bg-green-400' : ''} `}></p>
